...
- **Parallel `remember` calls clobbered each other** (#326): each `ctx_knowledge(action="remember")` did an unsynchronized read-modify-write of `knowledge.json`, so a burst of parallel MCP calls lost all but the last writer's fact. The read-modify-write is now serialized per project under a dedicated lock (`KNOWLEDGE_LOCKS`) and reloads the latest on-disk state inside the lock, so every concurrent fact is preserved.
